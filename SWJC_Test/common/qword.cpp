...
void QWord::mergeCells(int topLeftRow, int topLeftColumn, int bottomRightRow, int bottomRightColumn)
{
    QAxObject* selection = m_word->querySubObject("Selection");
    QAxObject* table = selection->querySubObject("Tables(1)");


    QAxObject* cell11 = table->querySubObject("Cell(int, int)", topLeftRow, topLeftColumn);
    QAxObject* cell19 = table->querySubObject("Cell(int, int)", bottomRightRow, bottomRightColumn);
    cell11->dynamicCall("Merge(QAxObject*)", cell19->asVariant());
